...
| DPF1 exact profile carrier V1 | Deep extension over P04; not a Session wire format | Contract implemented, activation blocked | One unsigned canonical framing implementation composes and verifies exact P04 artifacts, with an accepted-XNode golden vector, strict malformed corpus and exact reachable bounds. No production verifier/signer, persistence, network, DI, UI or activation is shipped. |
